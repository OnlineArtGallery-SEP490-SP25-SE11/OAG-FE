...
export default function Gallery() {
  ...
  const handleFilterArtist = (artists: string[]) => {
    if (artists.length === 0) {
      ...
      const filtered = artPieces.filter((art) => artists.includes(art.artist));
      setFilteredArtPieces(filtered);
    }
  };
    ...
    <TooltipProvider>
      <div className="flex">
        ...
        <div className="flex-1 p-4">
            ...
            <ResponsiveMasonry columnsCountBreakPoints={breakpointColumns}>
              <Masonry gutter="30px">
                {Array.from({ length: 8 }).map((_, index) => (
                  <div className="w-full" key={`${generateRandomKey()}`}>
                    <ArtPieceSkeleton height={getRandomHeight()} />
                  </div>
                ))}
              </Masonry>
            </ResponsiveMasonry>
              ...
              <Masonry gutter="30px">
                {filteredArtPieces.map((art, index) => (
                  <div
                    ...
                    ref={
                      index === filteredArtPieces.length - 1
                        ? lastArtPieceRef
                        : null
                    }
                  >
                    <ProductCard art={art} onClick={setSelectedArt} />
                  </div>
                ))}
              </Masonry>
            </ResponsiveMasonry>
          ...
          {loading && (
            <p className="text-center mt-4">Loading more art pieces...</p>
          )}
