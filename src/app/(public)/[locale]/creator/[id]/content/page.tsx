...
export default function ContentPage() {
	...
	const [contentByLanguage, setContentByLanguage] = useState<
		Record<string, ExhibitionContent>
	>(() => {
						...
						>
							<div>
								<Label htmlFor={`name-${lang.code}`}>
									Exhibition Name ({lang.name})
								</Label>
								<div className='mt-1'>
									<Input
										id={`name-${lang.code}`}
										value={
											contentByLanguage[lang.code].name
										}
										onChange={(e) =>
											handleContentChange(
												lang.code,
							...
							<div>
								<Label htmlFor={`description-${lang.code}`}>
									Short Description ({lang.name})
								</Label>
								<div className='mt-1'>
									<Textarea
										id={`description-${lang.code}`}
										value={
											contentByLanguage[lang.code]
												.description
										}
										onChange={(e) =>
											handleContentChange(
					...
					<div className='mt-2'>
						{welcomeImage ? (
							<div className='relative'>
								...
								<button
									onClick={() =>
										removeMedia(() => setWelcomeImage(null))
									}
							...
							<label className='flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50'>
								...
								<input
									...
									accept='image/*'
									onChange={(e) =>
										handleImageUpload(e, setWelcomeImage)
									}
								/>
							</label>
						)}
						...
						{backgroundMedia ? (
							...
							<label className='flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50'>
								<div className='flex flex-col items-center justify-center pt-5 pb-6'>
									...
									<p className='text-sm text-gray-500'>
										Click to upload background media
									</p>
								</div>
								<input
									type='file'
									...
									accept='image/*,video/*'
									onChange={(e) =>
										handleImageUpload(e, setBackgroundMedia)
									}
								/>
							</label>
						)}
						...
						{backgroundAudio ? (
							...
							<label className='flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50'>
								...
								<input
									...
									accept='audio/*'
									onChange={(e) =>
										handleImageUpload(e, setBackgroundAudio)
									}
								/>
							</label>
						)}
