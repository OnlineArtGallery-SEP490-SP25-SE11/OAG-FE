...
function ImageSection({ artwork, isZoomed, toggleZoom, imageLoaded, setImageLoaded, t }: ImageSectionProps) {
  return (
    <div
      ...
      {!imageLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/20">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-white/70 text-sm">{t('artwork.loading_image')}</p>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            {artwork.price > 0 && (
              <div className="bg-white/10 p-2 rounded-md flex items-center gap-1.5 col-span-1">
                ...
                {userHasPurchased && (
                  <Badge variant="outline" className="bg-green-500/20 text-green-300 border-green-500/30 px-1 py-0.5 text-[10px] ml-1">
                    {t('artwork.purchased')}
                  </Badge>
                )}
              </div>
          ...
          {artwork.category && artwork.category.length > 0 && (
            <div className='space-y-1'>
              <div className='flex items-center gap-1'>
                ...
                <span className='text-xs text-white/70'>{t('artwork.categories')}</span>
              </div>
          ...
          <div className="bg-white/5 rounded-md p-1.5 text-[10px] text-white/60">
            <div className="flex flex-wrap gap-y-1 gap-x-3 justify-between">
              <div className="flex items-center gap-1">
                ...
                <span>{artwork.dimensions?.width || 0}×{artwork.dimensions?.height || 0}</span>
              </div>
              <div className="flex items-center gap-1">
            ...
            Array.from({ length: commentsCount }).map((_, idx) => (
              ...
              >
                <div className='flex items-center gap-2 mb-2'>
                  ...
                  <div>
                    <p className='font-medium text-white text-sm'>{t('artwork.user')} {idx + 1}</p>
                    <p className='text-xs text-white/50'>{t('artwork.days_ago', { days: 2 })}</p>
                  </div>
                </div>
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      <AlertDialogContent className="bg-black/95 border border-white/20 text-white max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle className="text-center">{t('artwork.purchase_confirmation')}</AlertDialogTitle>
          <AlertDialogDescription className="text-white/80">
            <div className="space-y-4 my-4">
              <div className="border border-white/10 rounded-md p-4 bg-white/5">
                ...
                <p className="text-sm text-white/70 line-clamp-1">{t('artwork.artist')}: {artwork.artistId?.name || t('artwork.unknown_artist')}</p>
...
function Modal() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => selectedId ? fetchArtworkById(selectedId) : Promise.reject('Invalid ID'),
    enabled: !!selectedId
  });
  ...
  const purchaseMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (response.data?.success) {
        ...
        queryClient.invalidateQueries({ queryKey: ['userPurchased', selectedId] });
  ...
  }, [selectedId, accessToken, downloadToken, artwork?.title, t, toast, queryClient]);
      ...
      >
        ...
        >
          ...
          <button
            ...
            aria-label={alternativeLayout ? t('artwork.standard_view') : t('artwork.immersive_view')}
            title={alternativeLayout ? t('artwork.standard_view') : t('artwork.immersive_view')}
          >
