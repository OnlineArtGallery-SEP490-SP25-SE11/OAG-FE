...
export function useBlogs(searchTerm = '') {
    const getKey = (pageIndex: number, previousData: GetPublishedBlogsResponse | undefined) => {
  ...
  return {
    ...
    const blogs = data ? data.flatMap(page => page?.edges.map(edge => edge.node)) : [];
    ...
    return { blogs, isLoading, isError, size, setSize, isReachingEnd, isLoadingMore };
}